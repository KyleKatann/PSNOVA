...
HOME_STYLE = ROOT / "docs" / "css" / "home-product.css"
...
class HomepageProductMediaTests(unittest.TestCase):
    ...
    def test_css_styles_table_cells_instead_of_external_cards(self):
        css = HOME_STYLE.read_text(encoding="utf-8")
    ...
    def test_table_grid_is_owned_by_shared_css(self):
        ...
        self.assertNotIn("border-right: 1px solid var(--border);", home_css)
        self.assertNotIn("border-bottom: 1px solid var(--border);", home_css)
        self.assertNotIn(".official-links-table tr > :last-child", home_css)
