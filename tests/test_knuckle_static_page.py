...
class KnuckleStaticPageTests(unittest.TestCase):
    def test_knuckle_page_is_static_and_single_weapon(self):
        ...
        self.assertIn('data-weapon-static="true"', html)
        self.assertIn('data-weapon-type="ナックル"', html)
        self.assertNotIn("<details", html)
        self.assertNotIn("<summary", html)
        self.assertNotIn("<summary>アサルトライフル</summary>", html)
        self.assertNotIn("<summary>ソード</summary>", html)

    def test_knuckle_table_uses_static_semantics_and_reader_labels(self):
        html = KNUCKLE.read_text(encoding="utf-8")
        self.assertIn("<thead>", html)
        self.assertIn("<tbody>", html)
        self.assertIn('<th scope="col">ショップレベル</th>', html)
    ...
    def test_static_page_keeps_one_icon_and_static_navigation(self):
        html = KNUCKLE.read_text(encoding="utf-8")
        self.assertEqual(1, html.count('/PSNOVA/img/weapon/knuckle.png'))
        self.assertIn('/PSNOVA/pages/weapon/doublesaber.html', html)
        self.assertIn('/PSNOVA/pages/weapon/rifle.html', html)
        self.assertIn('/PSNOVA/pages/weapon.html', html)

    def test_weapon_tools_enhance_static_tables_without_deleting_them(self):
        js = TOOLS.read_text(encoding="utf-8")
        self.assertIn('table.weapon-data-table[data-weapon-static="true"]', js)
        self.assertIn("prepareLegacyChildPage", js)
        self.assertIn("if (!hasStaticChild) prepareLegacyChildPage", js)
