...
class WeaponIconOwnershipTests(unittest.TestCase):
    def test_detail_heading_icons_are_owned_by_static_html(self):
        for slug, icon in ICONS.items():
            with self.subTest(slug=slug):
                html = (WEAPON_DIR / f"{slug}.html").read_text(encoding="utf-8")

                self.assertEqual(
                    1,
                    html.count(f"/PSNOVA/img/weapon/{icon}"),
                )
    ...
    def test_runtime_weapon_icon_injection_is_removed(self):
        ...
        self.assertNotIn("weapon-icons.js", menubar)
        self.assertNotIn("var weaponIcons", image_layout)
        self.assertNotIn("native-icon-table", image_layout)
        self.assertNotIn("native-icon-table", shared_css)
