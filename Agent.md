...
- On desktop (`min-width: 801px`), weapon search/filter toolbars use `position: sticky` and remain visible near the top of the viewport after the reader scrolls to them. They use no card shadow. The weapon table header remains sticky below the toolbar; its sticky `top` offset must follow the toolbar's actual rendered height, top inset, and bottom margin rather than a hard-coded value such as `76px`. No table column may be fixed or horizontally sticky.
