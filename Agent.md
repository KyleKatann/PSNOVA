...
- 多数のrepository fileを監査する場合、1回のtool orchestration内でconnector call上限に達するまで連続取得しない。対象を小さい固定batchへ分割し、各batchのread結果を確認してから次へ進む。call上限到達で失敗したbulk監査を同じ形で再実行しない。
...
- 同名fileの別directory版、original/DLC版、patch版など複数pathを一括処理する場合、名前の規則からpathを推測して配列へ入れてはならない。`find`、repository tree、directory listing等のread-only結果で各pathの実在を確認し、確認できたfileだけを処理対象にする。
- GitHubのfile URLやraw URLを組み立てる場合、pathに半角スペースやその他URL予約文字が含まれるときは未エンコード文字列をそのまま渡さない。repository treeや取得済みpathを正として、各path segmentをURLエンコードしてからread-only fetchを行う。
...
- Pythonの`zipfile`でarchive内fileを読む場合、`unzip -l`が表示したエスケープ済みpathをそのまま`ZipFile.read()`へ渡してはならない。`namelist()`または`infolist()`から現在の実在名を取得し、その文字列だけをread対象に使う。
...
## 最優先ルール：ページのズームを禁止しない

**スマートフォン・タブレットを含むすべての公開ページで、ユーザーのピンチズーム、拡大、縮小を禁止または制限してはならない。** `meta[name="viewport"]` の `user-scalable=no`、`maximum-scale=1`、`minimum-scale=1` 等、`touch-action` によるピンチズーム抑止、gesture/touchイベントでの `preventDefault()`、広告・カルーセル・表・ナビゲーション等の局所要素上だけズームを無効化する指定を禁止する。広告を含むすべての領域でブラウザ標準のズーム操作を維持する。`touch-action` を指定する必要がある場合も、ピンチズームを阻害しない値にする。このルールを変更できるのは、ユーザーが将来この禁止自体を明示的に撤回した場合だけである。
