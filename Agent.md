...
**過去の引き継ぎ資料、古い会話、古いメモに記載されたHEADを、現在の停止判定基準として使用してはならない。** 停止判定に使える基準HEADは、直前の正常な書き込み、または直前のread-only確認で取得した最新HEADだけとする。自分自身がその後に正常にcommitを作成した場合は、そのcommit後のHEADを新しい基準HEADとして扱い、それ以前のHEADへ比較基準を戻してはならない。
