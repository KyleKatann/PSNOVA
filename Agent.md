...
**今回のPSNOVA公開サイトにおける「レアドロップの仕組みと、レアアイテムを最も落としやすくする方法」ページの編集に限り、未作成の「アイテム探知装置」個別ページへのリンクを仮想リンクとして先行設置してよい。** 「アイテム探知装置」のリンク先は `/PSNOVA/pages/item-detector.html` とし、既存の「エネミーレーダー」は `/PSNOVA/pages/enemy-radar.html` を使う。この許可は今回のレアドロップページ作業だけの例外であり、PSNOVA内の他ページや今後の作業で未作成ページへのリンクを一般的に許可するものではない。
